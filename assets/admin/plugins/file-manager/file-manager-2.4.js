...
$(document).on('click', '#file_manager_image #btn_img_delete', function () {
    ...
    $.ajax({
        type: "POST",
        url: VrConfig.baseURL + "/File/deleteImage",
    ...
    $('#file_manager_image .file-manager-content').on('scroll', function () {
        ...
        if (search.length < 1) {
            if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight) {
                ...
                $.ajax({
                    type: "POST",
                    url: VrConfig.baseURL + "/File/loadMoreImages",
                    data: setAjaxData(data),
                    success: function (response) {
                        setTimeout(function () {
                            var obj = JSON.parse(response);
...
$(document).on('input', '#input_search_image', function () {
    ...
    $.ajax({
        type: "POST",
        url: VrConfig.baseURL + "/File/searchImage",
        data: setAjaxData(data),
        success: function (response) {
            if (search.length > 1) {
                var obj = JSON.parse(response);
...
function refresh_quiz_images() {
    $.ajax({
        type: "POST",
        url: VrConfig.baseURL + "/File/getQuizImages",
        data: setAjaxData({}),
        success: function (response) {
            var obj = JSON.parse(response);
            if (obj.result == 1) {
...
$(document).on('click', '#file_manager_quiz_image #btn_quiz_img_delete', function () {
    ...
    $.ajax({
        type: "POST",
        url: VrConfig.baseURL + "/File/deleteQuizImage",
        data: setAjaxData(data),
        success: function (response) {
...
function select_quiz_image() {
    ...
    } else if (quiz_image_type == 'recipe') {
        var input = '<input type="hidden" name="list_item_image[]" value="' + quiz_img_default_file_path + '">' +
            '<input type="hidden" name="list_item_image_storage[]" value="' + quiz_img_storage + '">';
        if (data_is_update == 1) {
            input = '<input type="hidden" name="list_item_image_' + data_question_id + '" value="' + quiz_img_default_file_path + '">' +
                '<input type="hidden" name="list_item_image_storage_' + data_question_id + '" value="' + quiz_img_storage + '">';
        }
        var image = '<div class="quiz-question-image-container">' +
            input +
            '<img src="' + quiz_img_base_url + quiz_img_default_file_path + '" alt="">' +
    ...
    $('#file_manager_quiz_image .file-manager-content').on('scroll', function () {
        ...
        if (search.length < 1) {
            if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight) {
                ...
                $.ajax({
                    type: "POST",
                    url: VrConfig.baseURL + "/File/loadMoreQuizImages",
                    data: setAjaxData(data),
                    success: function (response) {
                        setTimeout(function () {
                            var obj = JSON.parse(response);
...
$(document).on('input', '#input_search_quiz_image', function () {
    ...
    $.ajax({
        type: "POST",
        url: VrConfig.baseURL + "/File/searchQuizImage",
        data: setAjaxData(data),
        success: function (response) {
            if (search.length > 1) {
                var obj = JSON.parse(response);
...
var recipe_data_is_update = '';
var recipe_item_id = '';
//update images
$('#file_manager_recipe_image').on('show.bs.modal', function (e) {
    recipe_data_is_update = $(e.relatedTarget).attr('data-is-update');
    recipe_item_id = $(e.relatedTarget).attr('data-list-item-id');
    refresh_recipe_images();
});

//select recipe image
$(document).on('click', '#file_manager_recipe_image .file-box', function () {
    $('#file_manager_recipe_image .file-box').removeClass('selected');
    $(this).addClass('selected');
    $('#selected_recipe_img_file_id').val($(this).attr('data-file-id'));
    $('#selected_recipe_img_file_path').val($(this).attr('data-default-file-path'));
    $('#selected_recipe_img_storage').val($(this).attr('data-file-storage'));
...
function refresh_recipe_images() {
    $.ajax({
        type: "POST",
        url: VrConfig.baseURL + "/File/getRecipeImages",
        data: setAjaxData({}),
        success: function (response) {
            var obj = JSON.parse(response);
            if (obj.result == 1) {
                document.getElementById("recipe_image_file_upload_response").innerHTML = obj.content;
            }
        }
    });
}

//delete recipe image file
$(document).on('click', '#file_manager_recipe_image #btn_recipe_img_delete', function () {
    var file_id = $('#selected_recipe_img_file_id').val();
    $('#recipe_img_col_id_' + file_id).remove();
    var data = {
        "file_id": file_id
    };
    $.ajax({
        type: "POST",
        url: VrConfig.baseURL + "/File/deleteRecipeImage",
        data: setAjaxData(data),
        success: function (response) {
...
//select recipe image file
$(document).on('click', '#file_manager_recipe_image #btn_recipe_img_select', function () {
    select_recipe_image();
});

//select recipe image on double click
$(document).on('dblclick', '#file_manager_recipe_image .file-box', function () {
    select_recipe_image();
});

function select_recipe_image() {
    var img_file_path = $('#selected_recipe_img_file_path').val();
    var img_storage = $('#selected_recipe_img_storage').val();
    var img_base_url = $('#selected_recipe_img_base_url').val();

    var input = '<input type="hidden" name="list_item_image[]" value="' + img_file_path + '">' +
        '<input type="hidden" name="list_item_image_large[]" value="">' +
        '<input type="hidden" name="list_item_image_storage[]" value="' + img_storage + '">';
    if (recipe_data_is_update == 1) {
        input = '<input type="hidden" name="list_item_image_' + recipe_item_id + '" value="' + img_file_path + '">' +
            '<input type="hidden" name="list_item_image_large_' + recipe_item_id + '" value="">' +
            '<input type="hidden" name="list_item_image_storage_' + recipe_item_id + '" value="' + img_storage + '">';
    }
    var image = '<div class="list-item-image-container">' +
        input +
        '<img src="' + img_base_url + img_file_path + '" alt="">' +
        '<a class="btn btn-danger btn-sm btn-delete-selected-file-image btn-delete-selected-list-item-image" data-image-type="recipe" data-list-item-id="' + recipe_item_id + '" data-is-update="' + recipe_data_is_update + '">' +
        '<i class="fa fa-times"></i> ' +
        '</a>' +
        '</div>';
...
//load more recipe images
jQuery(function ($) {
    $('#file_manager_recipe_image .file-manager-content').on('scroll', function () {
        var search = $("#input_search_recipe_image").val().trim();
        if (search.length < 1) {
            if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight) {
                var min = 0;
                $('#recipe_image_file_upload_response .file-box').each(function () {
                    var value = parseInt($(this).attr('data-file-id'));
                    if (min == 0) {
                        min = value;
                    }
                ...
                $.ajax({
                    type: "POST",
                    url: VrConfig.baseURL + "/File/loadMoreRecipeImages",
                    data: setAjaxData(data),
                    success: function (response) {
                        setTimeout(function () {
                            var obj = JSON.parse(response);
                            if (obj.result == 1) {
                                $("#recipe_image_file_upload_response").append(obj.content);
                            }
                        }, 100);
                    }
                });
...
//search recipe image
$(document).on('input', '#input_search_recipe_image', function () {
    var search = $(this).val().trim();
    var data = {
        "search": search
    };
    $.ajax({
        type: "POST",
        url: VrConfig.baseURL + "/File/searchRecipeImage",
        data: setAjaxData(data),
        success: function (response) {
            if (search.length > 1) {
                var obj = JSON.parse(response);
...
$(document).on('click', '#file_manager #btn_file_delete', function () {
    ...
    $.ajax({
        type: "POST",
        url: VrConfig.baseURL + "/File/deleteFile",
        data: setAjaxData(data),
        success: function (response) {
...
function refresh_files() {
    $.ajax({
        type: "POST",
        url: VrConfig.baseURL + "/File/getFiles",
        data: setAjaxData({}),
        success: function (response) {
            var obj = JSON.parse(response);
            if (obj.result == 1) {
    ...
    $('#file_manager .file-manager-content').on('scroll', function () {
        ...
        if (search.length < 1) {
            if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight) {
                ...
                $.ajax({
                    type: "POST",
                    url: VrConfig.baseURL + "/File/loadMoreFiles",
                    data: setAjaxData(data),
                    success: function (response) {
                        setTimeout(function () {
                            var obj = JSON.parse(response);
...
$(document).on('input', '#input_search_file', function () {
    ...
    $.ajax({
        type: "POST",
        url: VrConfig.baseURL + "/File/searchFiles",
        data: setAjaxData(data),
        success: function (response) {
            if (search.length > 1) {
                var obj = JSON.parse(response);
...
$(document).on('click', '#file_manager_video #btn_video_delete', function () {
    ...
    $.ajax({
        type: "POST",
        url: VrConfig.baseURL + "/File/deleteVideo",
        data: setAjaxData(data),
        success: function (response) {
...
function refresh_videos() {
    $.ajax({
        type: "POST",
        url: VrConfig.baseURL + "/File/getVideos",
        data: setAjaxData({}),
        success: function (response) {
            var obj = JSON.parse(response);
            if (obj.result == 1) {
    ...
    $('#file_manager_video .file-manager-content').on('scroll', function () {
        ...
        if (search.length < 1) {
            if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight) {
                ...
                $.ajax({
                    type: "POST",
                    url: VrConfig.baseURL + "/File/loadMoreVideos",
                    data: setAjaxData(data),
                    success: function (response) {
                        setTimeout(function () {
                            var obj = JSON.parse(response);
...
$(document).on('input', '#input_search_video', function () {
    ...
    $.ajax({
        type: "POST",
        url: VrConfig.baseURL + "/File/searchVideos",
        data: setAjaxData(data),
        success: function (response) {
            if (search.length > 1) {
                var obj = JSON.parse(response);
...
$(document).on('click', '#file_manager_audio #btn_audio_delete', function () {
    ...
    $.ajax({
        type: "POST",
        url: VrConfig.baseURL + "/File/deleteAudio",
        data: setAjaxData(data),
        success: function (response) {
...
function refresh_audios() {
    $.ajax({
        type: "POST",
        url: VrConfig.baseURL + "/File/getAudios",
        data: setAjaxData({}),
        success: function (response) {
            var obj = JSON.parse(response);
            if (obj.result == 1) {
    ...
    $('#file_manager_audio .file-manager-content').on('scroll', function () {
        ...
        if (search.length < 1) {
            if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight) {
                ...
                $.ajax({
                    type: "POST",
                    url: VrConfig.baseURL + "/File/loadMoreAudios",
                    data: setAjaxData(data),
                    success: function (response) {
                        setTimeout(function () {
                            var obj = JSON.parse(response);
...
$(document).on('input', '#input_search_audio', function () {
    ...
    $.ajax({
        type: "POST",
        url: VrConfig.baseURL + "/File/searchAudios",
        data: setAjaxData(data),
        success: function (response) {
            if (search.length > 1) {
                var obj = JSON.parse(response);
