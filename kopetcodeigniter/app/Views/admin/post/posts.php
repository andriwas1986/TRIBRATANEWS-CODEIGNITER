            ...
            <div class="col-sm-12">
                <div class="table-responsive">
                    <table class="table table-bordered table-striped" role="grid">
                        ...
                        <tbody>
                        <?php if (!empty($posts)):
                            // --- LOGIKA PENOMORAN URUT ---
                            $no = 1;
                            if (isset($pager) && method_exists($pager, 'getCurrentPage')) {
                                $no = 1 + ($pager->getCurrentPage() - 1) * $pager->getPerPage();
                            }

                            foreach ($posts as $item):
                                $language = getLanguage($item->lang_id);
                                $postSelections = getPostSelections($item->id);
                                $isSlider = false;
                                $isFeatured = false;
                                $isRecommended = false;
                                $isBreaking = false;
                                if (!empty($postSelections)) {
                                    foreach ($postSelections as $postSelection) {
                                        if ($postSelection->selection_type == 'slider') {
                                            $isSlider = true;
                                        }
                                        if ($postSelection->selection_type == 'featured') {
                                            $isFeatured = true;
                                        }
                                        if ($postSelection->selection_type == 'recommended') {
                                            $isRecommended = true;
                                        }
                                        if ($postSelection->selection_type == 'breaking') {
                                            $isBreaking = true;
                                        }
                                    }
                                } ?>
                                        ...
                                        <div class="td-post-item">
                                            <div class="post-image">
                                                <a href="<?= generatePostURL($item, generateBaseURLByLang($language)); ?>" target="_blank">
                                                    <div class="image">
                                                        <?php 
                                                            // FIX THUMBNAIL ADMIN PANEL
                                                            $adminImgUrl = getPostImage($item, "small");
                                                            if (empty($adminImgUrl)) { 
                                                                $adminImgUrl = $item->image_url; 
                                                            }
                                                            if (!empty($adminImgUrl) && strpos($adminImgUrl, 'http') === false) {
                                                                $adminImgUrl = base_url($adminImgUrl);
                                                            }
                                                        ?>
                                                        <img src="<?= IMG_BASE64_1x1; ?>" data-src="<?= $adminImgUrl; ?>" alt="" class="lazyload img-responsive" style="max-height: 80px; object-fit: cover;"/>
                                                    </div>
                                                </a>
                                            </div>
                                            <div class="post-title">
                                                ...
                                                <div class="preview">
                                                    ...
                                                    <?php endif;
                                                    if ($item->need_auth): ?>
                                                        <label class="label badge bg-warning label-table"><?= trans('only_registered'); ?></label>
                                                    <?php endif; ?>
                                                </div>
                                            </div>
                                        </div>
