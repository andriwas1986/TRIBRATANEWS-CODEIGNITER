...
<?php 
// =========================================================
// TARIK PENGATURAN HEADER DINAMIS DENGAN RESET & WARNA TEKS
// =========================================================
...
if ($db->tableExists('custom_header_settings')) {
    $hSet = $db->table('custom_header_settings')->where('id', 1)->get()->getRow();
    if (!empty($hSet)) {
        // Background Logo Banner
        if (!empty($hSet->bg_image)) {
            $logoBannerStyle = "background: url('".base_url($hSet->bg_image)."') center center / cover no-repeat;";
        } else if (!empty($hSet->bg_color)) {
            $logoBannerStyle = "background-color: ".$hSet->bg_color.";";
        }
        
        // Style & Text Color Top Bar
        if (!empty($hSet->top_menu_color)) {
            $topBarStyle = "background-color: ".$hSet->top_menu_color." !important;";
        }
        if (!empty($hSet->top_menu_text)) {
            $topBarTextColor = $hSet->top_menu_text;
        }

        // Style & Text Color Main Menu
        if (!empty($hSet->main_menu_color)) {
            $mainMenuStyle = "background-color: ".$hSet->main_menu_color." !important;";
        }
        if (!empty($hSet->main_menu_text)) {
            $mainMenuTextColor = $hSet->main_menu_text;
        }
    }
}
?>
...
<style>
    /* CSS Dinamis Top Bar */
    .top-bar { <?= $topBarStyle ?> }
    .top-bar a, .top-bar span, .top-bar li, .top-bar i { color: <?= $topBarTextColor ?> !important; }

    /* CSS Dinamis Logo Banner */
    .logo-banner { <?= $logoBannerStyle ?> }

    /* CSS Dinamis Main Menu */
    .main-menu, .navbar-default { <?= $mainMenuStyle ?> border: none !important; }
    .main-menu .navbar-nav > li > a { color: <?= $mainMenuTextColor ?> !important; transition: all 0.2s; }
    .main-menu .navbar-nav > li > a:hover { opacity: 0.8; }
    
    /* Perbaikan caret warna teks */
    .main-menu .navbar-nav > li > a .caret { border-top-color: <?= $mainMenuTextColor ?> !important; }
</style>
</head>
