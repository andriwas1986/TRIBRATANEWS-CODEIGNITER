...
// --- CUSTOM ADMIN PATH ---
// Mengubah jalur admin default menjadi 'pokok-awuren'
$customRoutes->admin = 'pokok-awuren';
// -------------------------

// ==========================================
// --- PWA ROUTES ---
// ==========================================
$routes->get('manifest.json', 'HomeController::manifest');
$routes->get('pwa-sw.js', 'HomeController::pwa_sw');
...
$routes->get('sites/fix_settings_database', 'SitesController::fix_settings_database');

/*
 * --------------------------------------------------------------------
 * Post Routes (Public Actions)
 * --------------------------------------------------------------------
 */
...
// Login Routes (mengikuti custom path 'pokok-awuren')
...
$routes->group($customRoutes->admin, ['filter' => 'auth'], function ($routes) {
    ...
    $routes->post('import-users', 'AdminController::importUsers');
    ...
    $routes->get('export-categories', 'CategoryController::exportCategories');
    $routes->post('import-categories', 'CategoryController::importCategories');

    // ==========================================
    // --- APP Tb News Route (SiBONDON) ---
    // ==========================================
    $routes->get('tb-news/export_excel', 'TbNewsController::exportExcel');
    $routes->get('tb-news', 'TbNewsController::index');
    $routes->post('tb-news/get_amplification_report', 'TbNewsController::get_amplification_report');

    // ==========================================
    // === MULTI SITE MANAGER ===
    // ==========================================
    $routes->get('multisite-setup', 'MultiSiteSetupController::index');
    $routes->post('multisite-setup/run', 'MultiSiteSetupController::run_setup');
    
    $routes->get('sites', 'SitesController::index');
    $routes->post('sites/add', 'SitesController::add_site');
    $routes->get('sites/delete/(:num)', 'SitesController::delete/$1');

    // ==========================================
    // --- IMPORT WORDPRESS ROUTE (FIXED & STABLE) ---
    // ==========================================
    $routes->get('import-wp', 'ImportController::index');
    $routes->post('import/process', 'ImportController::process');        // Legacy
    $routes->post('import/upload-xml', 'ImportController::uploadXml');   // Upload via AJAX
    $routes->post('import/run-batch', 'ImportController::runBatch');     // Proses Batch via AJAX
    $routes->post('import/undo', 'ImportController::undoImport');        // Reset Data
    $routes->get('import/debug', 'ImportController::debug');             // Diagnosa Server
    
    // [BARU] Route untuk Scanner XML (Mencegah Error 404 analyze-xml)
    $routes->post('import/analyze-xml', 'ImportController::analyzeXml'); 

    // [FIX] Menggunakan match() pengganti any() agar kompatibel semua versi CI4
    $routes->match(['get', 'post'], 'import/test-image', 'ImportController::testSingleImage'); 
    // ==========================================
    ...
    // 1. MEDSOS
    $routes->get('smart-report/medsos', 'MedsosController::index');
    $routes->post('smart-report/medsos/save', 'MedsosController::save'); 
    $routes->post('smart-report/medsos/save-engagement', 'MedsosController::saveDailyEngagement'); 
    $routes->post('smart-report/medsos/save-content', 'MedsosController::saveDailyContent'); 
    $routes->post('smart-report/medsos/delete-content-image', 'MedsosController::deleteContentImage'); 
    $routes->post('smart-report/medsos/save-settings', 'MedsosController::saveGlobalSettings'); 
    
    $routes->get('smart-report/medsos/export_pdf', 'MedsosController::exportPDF'); 
    $routes->get('smart-report/medsos/export_pptx', 'MedsosController::exportPPTX'); 
    $routes->get('smart-report/medsos/test_pptx', 'MedsosController::testPPTX'); 
    $routes->get('smart-report/medsos/view_report', 'MedsosController::viewReport'); 
    
    // 2. YOUTUBE
    $routes->get('smart-report/youtube', 'YoutubeController::index');
    $routes->post('smart-report/youtube/save', 'YoutubeController::save');
    $routes->post('smart-report/youtube/save-daily', 'YoutubeController::saveDailyStats');
    $routes->get('smart-report/youtube/export_excel', 'YoutubeController::exportExcel'); 

    // 3. POLICETUBE
    $routes->get('smart-report/policetube', 'PolicetubeController::index');
    $routes->post('smart-report/policetube/save', 'PolicetubeController::save');
    $routes->post('smart-report/policetube/save-daily', 'PolicetubeController::saveDailyStats');
    $routes->get('smart-report/policetube/export_excel', 'PolicetubeController::exportExcel'); 
    
    // ==========================================
    // TARUH DI SINI (Di dalam penutup grup Admin)
    $routes->get('mega-menu-settings', 'AdminController::megaMenuSettings');
...
foreach ($postRoutesArray as $item) {
    $array = explode('/', $item);
    if(isset($array[1])) $routes->post($item, $array[0] . 'Controller::' . $array[1]);
}
