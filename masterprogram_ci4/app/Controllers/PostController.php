    ...
    public function addPostPost()
    {
        ...
        if (!$this->validate(getValRules($val))) {
            ...
            if (!empty($postId)) {
                ...
                // --- [START] FIX SLIDER AUTO INSERT ---
                // Jika input add_to_slider bernilai 1, kita paksa masuk ke tabel post_selections
                if (inputPost('add_to_slider') == 1) {
                    $db = \Config\Database::connect();
                    $db->table('post_selections')->where('post_id', $postId)->where('selection_type', 'slider')->delete();
                    $db->table('post_selections')->insert([
                        'post_id' => $postId,
