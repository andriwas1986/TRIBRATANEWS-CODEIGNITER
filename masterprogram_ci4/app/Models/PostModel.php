...
class PostModel extends BaseModel
{
    ...
    public function buildQuery($langId = null, $fetchContent = false, $isPreview = false)
    {
        ...
        // FILTER SITE (SHARED CONTENT)
        if (defined('CURRENT_SITE_ID')) {
            $this->builder->groupStart();
                $this->builder->where('posts.site_id', CURRENT_SITE_ID);
                $this->builder->orWhere('posts.site_id', 1);
    ...
    {
        $cacheKey = 'category_posts_lang_' . $langId . '_site_' . (defined('CURRENT_SITE_ID') ? CURRENT_SITE_ID : 1);
        return getOrSetStableCache($cacheKey, function () use ($langId) {
    ...
    {
        $cacheKey = 'posts_latest_lang_' . $langId . '_' . $perPage . '_' . $offset . '_site_' . (defined('CURRENT_SITE_ID') ? CURRENT_SITE_ID : 1);
    ...
    //get selected posts (VERSI FINAL: FIX REFRESH SLIDER)
    public function getPostsSelected($langId)
    {
        // Ganti nama key cache (_v2) agar sistem terpaksa update data
        $cacheKey = 'posts_selected_lang_' . $langId . '_site_' . (defined('CURRENT_SITE_ID') ? CURRENT_SITE_ID : 1) . '_v2';
        
        return getOrSetCache($cacheKey, function () use ($langId) {
            $sql = "(SELECT ps.post_id, ps.selection_type FROM post_selections ps WHERE ps.selection_type = 'slider' ORDER BY ps.id DESC LIMIT 100) UNION ALL
                (SELECT ps.post_id, ps.selection_type FROM post_selections ps WHERE ps.selection_type = 'featured' ORDER BY ps.id DESC LIMIT 100) UNION ALL
                (SELECT ps.post_id, ps.selection_type FROM post_selections ps WHERE ps.selection_type = 'breaking' ORDER BY ps.id DESC LIMIT 100) UNION ALL
                (SELECT ps.post_id, ps.selection_type FROM post_selections ps WHERE ps.selection_type = 'recommended' ORDER BY ps.id DESC LIMIT 100)";
            
            $this->buildQuery($langId);
            return $this->builder->select('selections.selection_type')
                ->join("($sql) AS selections", 'posts.id = selections.post_id', 'inner')
                ->orderBy('posts.created_at DESC')
                ->get()->getResult();
        }, 60); // Cache diperpendek jadi 60 detik (1 menit)
    }

    //get popular posts
    public function getPopularPosts($langId)
    {
        $cacheKey = 'posts_popular_lang_' . $langId . '_site_' . (defined('CURRENT_SITE_ID') ? CURRENT_SITE_ID : 1);
        return getOrSetStableCache($cacheKey, function () use ($langId) {
    ...
    {
        $cacheKey = 'posts_count_' . $langId . '_site_' . (defined('CURRENT_SITE_ID') ? CURRENT_SITE_ID : 1);
        return getOrSetCache($cacheKey, function () use ($langId) {
            $this->buildQuery($langId);
    ...
    {
        $cacheKey = 'posts_lang_' . $langId . '_' . $perPage . '_' . $offset . '_site_' . (defined('CURRENT_SITE_ID') ? CURRENT_SITE_ID : 1);
        return getOrSetCache($cacheKey, function () use ($langId, $perPage, $offset) {
            $this->buildQuery($langId);
    ...
    {
        $cacheKey = 'posts_count_category_' . $categoryId . '_site_' . (defined('CURRENT_SITE_ID') ? CURRENT_SITE_ID : 1);
        return getOrSetCache($cacheKey, function () use ($categoryTree) {
    ...
    {
        $cacheKey = 'posts_category_' . $categoryId . '_' . $perPage . '_' . $offset . '_site_' . (defined('CURRENT_SITE_ID') ? CURRENT_SITE_ID : 1);
    ...
    {
        $cacheKey = 'rss_posts_lang_' . clrNum($this->activeLang->id) . '_site_' . (defined('CURRENT_SITE_ID') ? CURRENT_SITE_ID : 1);
        if (!empty($categoryId)) {
    ...
    public function getGoogleNewsFeeds($categories)
    {
        ...
        $cacheKey = 'google_news_lang' . $langId . '_cat' . $categoryId . '_author' . $userId . '_limit' . $limit . '_site_' . (defined('CURRENT_SITE_ID') ? CURRENT_SITE_ID : 1);
        $posts = cache($cacheKey);
        if (!empty($posts)) {
            return $posts;
        }
    ...
    // =========================================================================
    // [FIX DB] FUNGSI IMPORT
    // =========================================================================
    
    public function addPostImport($data)
    {
        if (empty($data['slug'])) {
    ...
    public function addImageImport($data)
    {
