...
class SettingsModel extends BaseModel
{
    ...
    {
        parent::__construct();
        // Builder di-init, tapi sebaiknya panggil table() langsung di fungsi update agar fresh
        ...
        // --- [MODIFIKASI MULTI-SITE] ---
        // Deteksi ID Website Aktif
    ...
    //update settings (LOGO, FAVICON, DST)
    public function updateSettings($langId)
    {
        // Pastikan ID tidak nol
        if (empty($this->activeSiteId)) {
            return false;
        }
        ...
        // [FIX UTAMA] Gunakan query builder fresh agar tidak cache kondisi sebelumnya
        $this->db->table('general_settings')
                 ->where('site_id', $this->activeSiteId)
                 ->update($general);
        ...
        // [FIX UTAMA] Update settings bahasa juga dikunci site_id
    ...
    public function updateRecaptchaSettings()
    {
        ...
        return $this->db->table('general_settings')->where('site_id', $this->activeSiteId)->update($data);
    }
    ...
    public function updatePreferences($form)
    {
        if ($form == 'general') {
            ...
            if (!empty($tempFile) && !empty($tempFile['path'])) {
                ...
                $pwaLogo = $currentSettings->pwa_logo ?? ''; 
    ...
    public function updateAIWriterSettings()
    {
        ...
        return $this->db->table('general_settings')->where('site_id', $this->activeSiteId)->update(['ai_writer' => serialize($data)]);
    }
    ...
    public function updateSeoSettings()
    {
        ...
        if ($submit == 'google_analytics') {
            ...
            return $this->db->table('settings')->where('lang_id', clrNum($langId))->where('site_id', $this->activeSiteId)->update($data);
        }
        return true;
    }
    ...
    {
        return $this->db->table('settings')->where('lang_id', clrNum($langId))->where('site_id', $this->activeSiteId)->get()->getRow();
    }
    ...
    public function setThemeMode()
    {
        ...
        if ($mode == 'light' || $mode == 'dark') {
            ...
            $this->db->table('general_settings')->where('site_id', $this->activeSiteId)->update(['theme_mode' => $mode]);
        }
    }
    ...
    {
        // Note: Logic tema saat ini bersifat GLOBAL karena struktur tabel 'themes' belum mendukung site_id.
        // Jika ingin tema beda per site, perlu struktur tabel tambahan.
    ...
    {
        return $this->db->table('general_settings')->where('site_id', $this->activeSiteId)->update(['last_cron_update' => date('Y-m-d H:i:s')]);
    }
