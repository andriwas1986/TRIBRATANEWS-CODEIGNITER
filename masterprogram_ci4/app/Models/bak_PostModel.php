...
class PostModel extends BaseModel
{
    ...
    public function buildQuery($langId = null, $fetchContent = false, $isPreview = false)
    {
        ...
        // =====================================================================
        // --- [MODIFIKASI FILTER SITE: SHARED CONTENT MODE] ---
        // =====================================================================
        // Logika: Tampilkan postingan milik Site ini ATAU milik Site Utama (ID 1)
        if (defined('CURRENT_SITE_ID')) {
            $this->builder->groupStart();
                $this->builder->where('posts.site_id', CURRENT_SITE_ID); // Milik Subdomain
                $this->builder->orWhere('posts.site_id', 1);             // Milik Pusat (Shared)
    ...
    {
        // Cache key harus unik per site agar tidak tercampur
    ...
    {
        $cacheKey = 'posts_selected_lang_' . $langId . '_site_' . (defined('CURRENT_SITE_ID') ? CURRENT_SITE_ID : 1);
    ...
    {
        // Fungsi ini akan memanggil buildQuery(), jadi filter site_id OR site_id=1 sudah otomatis berlaku
