<div class="row">
            ...
            <form action="<?= base_url('Admin/cacheSystemPost'); ?>" method="post">
                ...
                <div class="card-body">
                    ...
                    <div class="card-footer" style="padding-left: 0; padding-right: 0;">
                        ...
                        <button type="submit" name="action" value="reset" class="btn btn-warning float-end m-r-10"><?= trans('reset_cache'); ?></button>
                    </div>
                </div>
            </form>
        </div>
    ...
    <div class="col-lg-6 col-md-12">
        <div class="card card-primary">
            <div class="card-header with-border">
                <h3 class="card-title"><?= trans("static_cache_system"); ?></h3>
            </div>
            <form action="<?= base_url('Admin/cacheSystemPost'); ?>" method="post">
                <?= csrf_field(); ?>
                <div class="card-body">
                    <div class="form-group">
                        <label><?= trans("status"); ?></label>
                        <?= formRadio('static_cache_system', 1, 0, trans("enable"), trans("disable"), $generalSettings->static_cache_system); ?>
                    </div>
                    <div class="card-footer" style="padding-left: 0; padding-right: 0;">
                        <button type="submit" name="action" value="save_static" class="btn btn-primary float-end"><?= trans('save_changes'); ?></button>
                        <button type="submit" name="action" value="reset_static" class="btn btn-warning float-end m-r-10"><?= trans('reset_cache'); ?></button>
                    </div>
                </div>
            </form>
        </div>
