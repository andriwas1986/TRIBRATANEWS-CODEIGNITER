        ...
        <div class="form-group m-0">
            <?php if (!empty($post)):
                if (empty($post->image_id)) {
                    ...
                    $postImgURL = getPostImage($post, 'mid');
                }
                
                // --- FIX: Otomatis tambahkan base_url jika path lokal ---
                if (!empty($postImgURL) && strpos($postImgURL, 'http') === false) {
                    $postImgURL = base_url($postImgURL);
                }
                // --------------------------------------------------------
                ?>
                    ...
                    <div class="col-sm-12">
                        <?php if (!empty($postImgURL)): ?>
                            <div id="post_select_image_container" class="post-select-image-container">
                                <img src="<?= $postImgURL; ?>" id="selected_image_file" alt="" style="max-height: 250px; object-fit: contain;">
                            ...
                            <div id="post_select_image_container" class="post-select-image-container">
                                <a class="btn-select-image" data-toggle="modal" data-target="#file_manager_image" data-image-type="main">
                                    <div class="btn-select-image-inner">
                                        ...
                                        <button type="button" class="btn"><?= trans("select_image"); ?></button>
                                    </div>
                                </a>
                            </div>
                        <?php endif; ?>
