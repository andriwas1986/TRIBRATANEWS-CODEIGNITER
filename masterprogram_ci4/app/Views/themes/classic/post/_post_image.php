<?php if (!empty($postItem) && !empty($type)):
    ...
    // --- FIX: Fungsi Otomatis Tambah Domain Untuk Gambar Impor ---
    if (!function_exists('getSafeImageUrl')) {
        function getSafeImageUrl($post, $size) {
            $url = getPostImage($post, $size);
            // Jika bukan link luar (http) dan isinya tidak kosong, tempelkan domain
    ...
    if ($type == 'featured_slider'): ?>
        ...
        <div class="img-container">
            <img src="<?= IMG_BASE64_600x460; ?>" data-lazy="<?= getSafeImageUrl($postItem, 'slider'); ?>" alt="<?= esc($postItem->title); ?>" class="img-cover" width="600" height="460"/>
        </div>
    <?php elseif ($type == 'random_slider'): ?>
        <img src="<?= IMG_BASE64_360x215; ?>" alt="bg" class="img-responsive img-bg" width="360" height="215"/>
        <div class="img-container">
            <img src="<?= IMG_BASE64_360x215; ?>" data-lazy="<?= getSafeImageUrl($postItem, 'mid'); ?>" alt="<?= esc($postItem->title); ?>" class="img-cover" width="360" height="215"/>
        </div>
    <?php else:
        if ($type == 'featured') {
        ...
        // Panggil fungsi FIX di atas
        $imageDataSrc = getSafeImageUrl($postItem, $imageSize);
        
        $isGif = false;
        if (!empty($imageDataSrc) && strtolower(pathinfo($imageDataSrc, PATHINFO_EXTENSION)) === 'gif') {
            $isGif = true;
        }
        if (!empty($postItem->image_url) || $isGif == true || $type == 'featured'): ?>
            <img src="<?= $imgBg; ?>" alt="bg" class="img-responsive img-bg"<?= !empty($imgSz) ? $imgSz : ''; ?>/>
            <div class="img-container">
                <img src="<?= $imgBg; ?>" data-src="<?= $imageDataSrc; ?>" alt="<?= esc($postItem->title); ?>" class="lazyload img-cover"<?= !empty($imgSz) ? $imgSz : ''; ?>/>
            </div>
        <?php else: ?>
            <img src="<?= $imgBg; ?>" data-src="<?= $imageDataSrc; ?>" alt="<?= esc($postItem->title); ?>" class="lazyload img-responsive img-post"<?= !empty($imgSz) ? $imgSz : ''; ?>/>
        <?php endif;
    endif;
endif; ?>